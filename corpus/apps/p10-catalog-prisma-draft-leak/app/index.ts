 ...
 *
 * This is the wrong one of the pair. A draft is hidden by the route that reads one
 * product and listed by the route that reads them all.
 */
...
/**
 * The listing returns the catalog.
 *
 * `visibleTo` exists and the detail route below calls it. This one does not, because the
 * listing was written first, when everything in the catalog was published, and the draft
 * flag arrived later with the route that reads one product.
 */
app.get('/api/products', (_request: Request, response: Response) => {
